...
let LocalUnicodeObj: Record<string, string> = {};
/**
...
export function createSVG(options: SvgToFontOptions = {}): Promise<Record<string, string>> {
  ...
  UnicodeObj = {}
  const dataStr = fs.readFileSync(process.cwd() + '/db.dat', {encoding: 'utf-8'})
  if (dataStr == null || dataStr.length == 0) {
    LocalUnicodeObj = {}
  } else {
    LocalUnicodeObj = JSON.parse(dataStr)
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    fontStream.pipe(fs.createWriteStream(DIST_PATH))
      .on("finish", () => {
        ...
        fs.writeFileSync(process.cwd() + '/db.dat', JSON.stringify(LocalUnicodeObj), {encoding: 'utf-8'})
        resolve(UnicodeObj);
      })
      .on("error", (err) => {
        if (err) {
...
function getIconUnicode(name: string, useNameAsUnicode: boolean) {
  let res = LocalUnicodeObj[name]
  if (res != null && res.length !== 0) {
    UnicodeObj[name] = res
    return [res]
  }
  if (useNameAsUnicode) {
    UnicodeObj[name] = name
    return [name]
  }
  let unicode = String.fromCharCode(startUnicode++);
  while (true) {
    let hasRepeat = false
    Object.keys(LocalUnicodeObj).forEach(key => {
      if (LocalUnicodeObj[key] == unicode) {
        hasRepeat = true
      }
    })
    if (!hasRepeat) {
